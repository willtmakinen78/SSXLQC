...
# calls upon the _ConveyerHandler class, initiating directory observation
class Watcher:
    ...
    # begin monitoring
    def run(self):
        event_handler = ConveyerHandler()
        self.observer.schedule(event_handler, self.DIRECTORY_TO_WATCH, recursive=False)
        self.observer.start()
        try:
            print("conveyer folder monitor starting")
            while True:
                time.sleep(5)
        except KeyboardInterrupt:
            self.observer.stop()
            print("conveyer watcher exiting")

        self.observer.join()

# catches any files uploaded and initites analysis
class ConveyerHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        ...
        # called when a file is first created/moved to tge folder
            ...
            conveyer_data = ConveyerData(from_location)

            file_info = ConveyerSummary.parse_filename(ConveyerSummary, from_location)
            info_length = len(file_info)
            conveyer_data.unpack()
            summary = conveyer_data.get_summary()
            conveyer_data.write_trial_summary()

            # generate the output filename/location
            for i in range(len(from_location) - 1, 0, -1):
                if from_location[i] == "\\":
                    to_location = from_location[:i] + "\\summary\\SSXL_Conveyer_Summary.csv"
                    break
            print("updating conveyer summary at: " + to_location)
            ...
            plot(data, title_line, info_length + 3, 3, NUM_STATS, 
                 "save", to_location[:-34], sup_title="Conveyer",
                  grid_size=(4, 3), title_strip=-7)